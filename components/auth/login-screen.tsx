...
export function LoginScreen({
  ...
  return (
    <div className="mx-auto flex min-h-dvh max-w-[27.5rem] flex-col bg-bg px-6">
      <div className="flex flex-1 flex-col items-center justify-center text-center">
        ...
        <h1 className="mt-5 text-[1.75rem] leading-8 font-semibold text-fg">{t.auth.title}</h1>
        <p className="mt-2 text-body text-fg-secondary">{t.auth.subtitle}</p>

        {error && (
          <div className="mt-5 w-full rounded-tg bg-danger/10 px-4 py-3">
            <p className="text-body font-medium text-danger">{t.auth.error}</p>
            <p className="mt-0.5 text-caption text-fg-secondary">{t.auth.errorHint}</p>
          </div>
        )}
      </div>
      ...
      <div className="pb-[calc(2rem+env(safe-area-inset-bottom))]">
        {configured && botUsername ? (
          <>
            ...
            <p className="mt-3 text-center text-caption text-fg-tertiary">{t.auth.privacy}</p>
          </>
        ) : devLoginAllowed ? (
          <DevLoginButton />
        ) : (
          <div className="rounded-tg bg-fill px-4 py-3 text-center">
            <p className="text-body text-fg">{t.auth.notConfigured}</p>
            <p className="mt-1 text-caption text-fg-secondary">{t.auth.notConfiguredHint}</p>
          </div>
        )}
      </div>
    </div>
